...
import MasterStoreModal from 'src/components/forms/MasterStoreModal';

const MastersStorePage = () => {
  const [rows, setRows] = useState([]);
  ...
  const getTableData = async () => {
    setLoading(true);
    try {
      const { data } = await masterStoreGetAll();
      console.log(data.data);
      if (data) {
        const arrayOfRows = data.data.map((item) => ({
          col1: item?.store_id ? item?.store_id : '',
          col2: item?.businessId ? item?.businessId : '---',
          col3: item?.city ? item?.city : '---',
          col4: item?.userType ? item?.userType : '---',
          col5: item.province ? item.province : '---',
          col6: item?.approvalStatus ? item?.approvalStatus : '---',
          col7: (
  ...
  const deleteTableData = async () => {
    ...
    const { message } = await masterStoreDelete(tableId);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <TableComponent data={rows} columns={CONSTANTS.MASTER_STORE_HEADERS} />
      )}
      {deleteModal && (
        <DialogModal
...
export default MastersStorePage;
