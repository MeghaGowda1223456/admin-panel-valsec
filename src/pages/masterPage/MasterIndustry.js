...
import MasterIndustryModal from 'src/components/forms/MasterIndustryModal';

const MastersIndustryPage = () => {
  const [rows, setRows] = useState([]);
  ...
  const getTableData = async () => {
    setLoading(true);
    try {
      const { data } = await masterIndustryGetAll();
      console.log(data.data);
      if (data) {
        const arrayOfRows = data.data.map((item) => ({
          col1: item?.industry_id ? item?.industry_id : '---',
          col2: item?.industryName ? item?.industryName : '---',
          col3: item?.transactionDate ? item?.transactionDate : '---',
  ...
  const deleteTableData = async () => {
    ...
    const { message } = await masterIndustryDelete(tableId);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <TableComponent data={rows} columns={CONSTANTS.MASTER_INDUSTRY_HEADERS} />
      )}
      {deleteModal && (
        <DialogModal
...
export default MastersIndustryPage;
